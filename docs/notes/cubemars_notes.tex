...
\title{\textbf{CubeMars AK80-9 KV60}\\[2pt]
...
Output torque (cont.) & $\approx$7.5\,N$\cdot$m \\
Output torque (peak)  & 18\,N$\cdot$m \\
Output speed (no-load)& $\approx$52\,rad/s at 48\,V \\
...
For AK80-9: $P_{\max}=12.5$\,rad, $V_{\max}=50$\,rad/s,
$T_{\max}=18$\,N$\cdot$m.
...
\midrule
R-Link setting  & CAN Mode = \textbf{MIT}
...
\midrule
CAN Mode    & Periodic Feedback & \textbf{MIT} \\
...
\subsection{Recommended gains for AK80-9}
...
Torque (open-loop)   & 0       & 0         & Max 1\,N$\cdot$m unloaded \\
\bottomrule
\end{tabular}
\end{center}
...
\item \textbf{[CURRENT ISSUE] Motor is in Servo Mode.}
  ...
  \textbf{Fix:} R-Link $\to$ CAN Mode = MIT $\to$ Write $\to$ power-cycle.

\item \textbf{Motor ESC\_ID is 104, not 4.}
  R-Link showed ``CAN~ID: 4'' in the Application Configuration panel
  during initial bring-up, but this referred to a stale servo-mode
  configuration.  The actual ESC\_ID embedded in the extended-CAN
  feedback frame is 104 (\texttt{0x68}).  \texttt{motor\_config.py}
  has been corrected to \texttt{can\_id=0x68}.

\item \textbf{No firmware parameter read over CAN.}
  Unlike Damiao, the AK80-9 does not expose \texttt{PMAX}, \texttt{VMAX},
  or \texttt{TMAX} via CAN.  These are hard-coded in
  \texttt{CUBEMARS\_LIMITS["AK80-9"] = [12.5, 50.0, 18.0]}
  and must be confirmed against the datasheet if the firmware is updated.
